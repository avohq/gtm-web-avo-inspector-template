...
const onsuccess = () => {
   ...
   if (!initializedBy) {
    ...
    //
    // Same lookup contract as the already-initialized branch: the triggering
    // push may still be deferred when a cached script callback arrives, so
    // retry, and report the tag outcome only once the lookup has finished.
...
- name: A later dataLayer push does not steal the second instance's triggering event
  code: |-
    ...
    assertThat(capturedCalls[2].args[0]).isEqualTo('test_event');
    assertThat(capturedCalls[2].args[2].outputReference).isEqualTo('ga4-9f3');
- name: Later fire sends the event when its push is already in the dataLayer
  code: |-
...
- name: Later fire retries until a deferred push lands in the dataLayer
  code: |-
    mock('getContainerVersion', function() { return { previewMode: false }; });
    mockObject('templateStorage', { getItem: function(key) { return true; }, setItem: function(key, value) {} });
    mock('copyFromDataLayer', function(key) {
